const defaultAuthorizedUserEmails = [
  "ashorangelake@gmail.com",
  "ocampojenis333@gmail.com",
  "brian.caceres001@gmail.com",
  "skmehedihasan.jr1@gmail.com",
];

const configuredAuthorizedUserEmails = (
  process.env.NEXT_PUBLIC_AUTHORIZED_USER_EMAILS ??
  defaultAuthorizedUserEmails.join(",")
)
  .split(",")
  .map((email) => email.trim().toLowerCase())
  .filter(Boolean);

export function isAuthorizedUserEmail(email: string): boolean {
  return configuredAuthorizedUserEmails.includes(email.trim().toLowerCase());
}

export const unauthorizedUserMessage =
  "This email address is not authorized to access this site.";
